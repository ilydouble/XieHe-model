...
| Pose Corner | `weights/pose_corner.pt` | 椎体角点检测，每个4角点: TL, TR, BR, BL |

### 椎体编号规则

Pose Corner 模型输出的椎体编号**不依赖**模型预测的原始类别 ID，而是在推理时自动处理：

1. **IoU 去重**：同一位置被重复检测时（不同类别预测到相同位置），保留置信度最高的一个
2. **按 y 坐标排序**：从上到下依次编号 V0, V1, V2, ...
   - `V0` = 图像中最靠上的椎体（通常为 C7）
   - `V1` = 第二节（通常为 T1）
   - 以此类推

这样可以避免模型类别预测偏移或重复导致的错误。
