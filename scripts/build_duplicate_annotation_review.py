...
$('prev').onclick=()=>move(-1);$('next').onclick=()=>move(1);$('filter').onchange=()=>{current=0;render()};$('jump').onchange=()=>{const n=Number($('jump').value),i=filtered.findIndex(g=>g.number===n);if(i>=0){current=i;render()}else{const target=pkg.groups[n-1];if(target){$('filter').value='all';rebuildFilter();current=n-1;render()}}};$('note').oninput=()=>{const g=filtered[current];state[g.id]={...(state[g.id]||{}),note:$('note').value,updated_at:new Date().toISOString()};save()};
