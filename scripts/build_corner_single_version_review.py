...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成单版本角点抽样确认包")
    parser.add_argument("--dataset-root", required=True, type=Path)
    parser.add_argument("--matched-candidates", required=True, type=Path, help="服务器-当前同图候选TSV")
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--count", type=int, default=30)
    parser.add_argument("--dataset-role", choices=("current743", "server891"), default="current743")
...
def remaining_images(dataset_root: Path, matched_candidates: Path, dataset_role: str = "current743") -> list[Path]:
    matched_column = 0 if dataset_role == "server891" else 1
    matched = {
        Path(line.split("\t")[matched_column]).resolve()
    ...
    return sorted(
        (
            ...
            if (dataset_role == "server891" or not path.name.startswith("eap_"))
            and path.resolve() not in matched
        ),
...
def deterministic_stratified_sample(images: list[Path], count: int) -> list[Path]:
    ...
    for image in images:
        groups[image.parent.name].append(image)
    raw = {split: count * len(group) / len(images) for split, group in groups.items()}
    allocation = {split: int(value) for split, value in raw.items()}
    for split in sorted(groups, key=lambda name: (raw[name] - allocation[name], len(groups[name]), name), reverse=True)[:count - sum(allocation.values())]:
        allocation[split] += 1
    selected: list[Path] = []
    for split in sorted(groups):
        ordered = sorted(groups[split], key=lambda path: hashlib.sha256(path.name.encode()).hexdigest())
        amount = allocation[split]
        if amount == 0:
            continue
        positions = [round(index * (len(ordered) - 1) / (amount - 1)) for index in range(amount)] if amount > 1 else [len(ordered) // 2]
...
def render(image_path: Path, label_path: Path, output: Path, rank: int, dataset_role: str = "current743") -> None:
    ...
    bands = (
        [(range(0, 6), "C7–T5"), (range(6, 13), "T6–T12"), (range(13, 18), "L1–L5")]
        if dataset_role == "server891"
        else [(range(1, 7), "T1–T6"), (range(7, 13), "T7–T12"), (range(13, 18), "L1–L5")]
    )
    ...
    title = "服务器版净新增样本" if dataset_role == "server891" else "743/NRRD版未重叠样本"
    d.text((18, 12), f"#{rank:02d}  {title}", fill="white", font=font(26))
...
def build(dataset_root: Path, matched_candidates: Path, output_dir: Path, count: int, dataset_role: str = "current743") -> list[dict[str, str]]:
    ...
    for rank, image in enumerate(selected, 1):
        ...
        preview = Path("previews") / f"{rank:02d}_{image.stem}.jpg"; render(image, label, output_dir / preview, rank, dataset_role)
    ...
    write_html(output_dir / "打开人工确认页面.html", rows, dataset_role)
    if dataset_role == "server891":
        heading = "服务器891份净新增角点抽样确认包"
        scope = f"服务器去重后的1381份排除与旧743同影像的490份后剩余{len(all_remaining)}份。"
        display = "整图标出C7-L5，右侧分别放大C7-T5、T6-T12、L1-L5。"
    else:
        heading = "743版未重叠角点抽样确认包"
        scope = f"743份排除与服务器同影像的490份后剩余{len(all_remaining)}份。"
        display = "整图标出V1-V17，右侧分别放大T1-T6、T7-T12、L1-L5。"
    (output_dir / "README.md").write_text(f"# {heading}\n\n{scope}本包按稳定哈希顺序均匀抽取{len(rows)}份，覆盖不同文件名与检查，非按标注质量筛选。\n\n{display}打开HTML选择整体正确、编号错位、角点不准、两者都不对或无法判断。选择自动保存在浏览器本地。原数据未修改。\n", encoding="utf-8")
    return rows


def main() -> None:
    args = parse_args(); rows = build(args.dataset_root.resolve(), args.matched_candidates.resolve(), args.output_dir.resolve(), args.count, args.dataset_role)
